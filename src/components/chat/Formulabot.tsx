...
const QUICK = [
  {
    label: "Jelaskan hasil prediksi",
    text: "Jelaskan hasil prediksi formula ini dengan bahasa formulator: apa arti angkanya, interval, dan apa yang perlu diperhatikan.",
  },
  {
    label: "Bahan mana yang berisiko",
    text: "Bahan mana di formula ini yang berisiko untuk stabilitas, keamanan, atau kepatuhan, dan kenapa. Urutkan dari yang paling penting.",
  },
  {
    label: "Usulkan perbaikan formula",
    text: "Usulkan perbaikan formula ini agar memenuhi target QTPP: bahan yang perlu diubah, alternatifnya, dan rentang konsentrasi yang wajar.",
  },
  {
    label: "Ringkas untuk manajer",
    text: "Buat ringkasan singkat untuk manajer R&D: status formula, angka kunci, risiko utama, dan langkah berikutnya.",
  },
];
    ...
    try {
      await streamChat(
        ...
        (delta) =>
          setMsgs((m) =>
            m.map((x, i) =>
              i === m.length - 1 ? { ...x, content: x.content + delta } : x,
            ),
          ),
      ...
      if (!(e instanceof DOMException && e.name === "AbortError"))
        setError(
          e instanceof Error ? e.message : "Formulabot tidak bisa dihubungi.",
        );
    } finally {
      setBusy(false);
      abort.current = null;
        ...
        >
          <header className="flex items-center gap-3 border-b-2 border-line px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[16px] font-bold leading-tight text-navy">
                Formulabot
              </p>
              <p className="truncate text-[11px] font-semibold text-grey-text">
                {ws
                  ? `konteks: ${ws.name}, langkah ${ws.step}`
                  : "konteks: belum ada workspace"}
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="relative flex min-h-0 flex-1 flex-col">
            ...
            <img
              ...
              alt=""
              aria-hidden="true"
              className="pointer-events-none absolute -bottom-6 -right-8 w-[240px] opacity-[0.12]"
            />
            <div
              ref={list}
              role="log"
            ...
            >
              {msgs.length === 0 && (
                <p className="rounded-[10px] border border-line bg-white px-3 py-2 text-[13px] font-semibold text-grey-text">
                  Halo, saya Formulabot. Saya membaca formula, QTPP, dan hasil
                  prediksi di workspace ini. Tanya apa saja, atau pakai tombol
                  cepat di bawah.
                </p>
              )}
              {msgs.map((m, i) => (
                ...
                >
                  <p
                    className={`max-w-[88%] whitespace-pre-wrap rounded-[14px] px-3 py-2 text-[13px] font-medium leading-snug ${m.role === "user" ? "bg-blue text-white" : "border border-line bg-white text-black"}`}
                  >
                    {m.content ||
                      (busy && i === msgs.length - 1 ? "Sedang berpikir…" : "")}
                  </p>
                </div>
              ))}
              {error && (
                <p
                  role="alert"
                  className="flex flex-wrap items-center gap-2 text-[12px] font-semibold text-bad"
                >
            ...
            <div className="mb-2 flex flex-wrap gap-1">
              {QUICK.map((q) => (
                <button
                  ...
                  disabled={busy}
                  onClick={() => send(q.text)}
                  className="chip h-7 px-2 text-[11px] hover:bg-line disabled:opacity-50"
                >
                  {q.label}
                </button>
              ))}
            ...
            >
              ...
              {busy ? (
                <button
                  type="button"
                  onClick={stop}
                  className="btn-outline h-[44px] min-h-0 px-4 text-[13px]"
                >
                  Stop
                </button>
              ) : (
                <button
                  type="submit"
                  className="btn-primary h-[44px] min-h-0 px-4 text-[13px]"
                  disabled={!input.trim()}
                >
                  Kirim
                </button>
