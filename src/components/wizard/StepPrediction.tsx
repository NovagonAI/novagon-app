  ...
  return (
    <div>
      <Panel title="Hasil Prediksi" bodyClassName="pt-[16px] pb-[26px]">
        <p className="text-[20px] font-bold text-navy">Nilai Prediksi</p>
        {h ? (
          <>
        ...
        <Panel title="Catatan Model" className="mt-[22px]" bodyClassName="pt-[16px] pb-[22px]">
          {h && (
            <p className="text-[14px] font-semibold text-grey-text">
              {h.label} · {HEAD_LABEL[h.head]} ({h.head}){h.raw.provenance.attribution ? ` · ${h.raw.provenance.attribution}` : ''}
            </p>
          )}
          {others.length > 0 && (
          ...
          {Object.keys(a.problems).length > 0 && (
      ...
      <div className="mt-[18px] flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-3">
          ...
          <button type="button" onClick={rerun} disabled={busy} className="btn-outline">
